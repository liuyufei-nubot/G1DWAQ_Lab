...
from legged_lab.envs.g1.g1_vision_config import G1VisionEnvCfg, G1VisionStairsEnvCfg, G1VisionRoughEnvCfg
...
class G1VisionEnv(VecEnv):
    """
    G1 Vision Environment with Depth Anything V2 visual features.
    
    该环境基于 G1RgbEnv，使用 Depth Anything V2 提取视觉特征:
    - RGB 相机采集图像
    - DepthAnythingEncoder 提取 CLS token (384-dim)
    - VisionFeatureManager 降频更新 (每5步)
    - 视觉特征通过 extras 传递给 Runner
    
    架构设计:
    - 视觉 encoder 与环境解耦，在 Runner 层管理 (推荐方式)
    - 或者在环境内部管理 encoder (本实现支持两种模式)
    - Critic 使用特权信息 (height_scan, feet_info)
    - Actor 使用 RGB 视觉特征 + 本体感知
    
    与 G1RgbEnv 的区别:
    - 不直接输出 RGB 图像，而是输出预提取的视觉特征
    - 支持降频视觉更新，模拟真实部署场景
    - 使用专门的 VisionOnPolicyRunner 进行训练
    """

    def __init__(
        self,
        cfg: G1VisionEnvCfg | G1VisionStairsEnvCfg | G1VisionRoughEnvCfg,
        headless: bool,
    ):
        self.cfg: G1VisionEnvCfg | G1VisionStairsEnvCfg | G1VisionRoughEnvCfg
        ...
        self.vision_cfg = self.cfg.vision
        self.use_vision = self.vision_cfg.enable
        self.vision_feature_dim = self.vision_cfg.feature_dim
        self.vision_update_interval = self.vision_cfg.update_interval
        ...
        # Instantiate RGB Camera (required for vision)
        if hasattr(self.cfg.scene, "rgb_camera") and self.cfg.scene.rgb_camera.enable_rgb_camera:
            ...
            if self.use_vision:
                print("[WARNING] Vision is enabled but RGB camera is not configured!")
        ...
        # Initialize vision encoder (optional - can also be managed by runner)
    ...
    def _init_vision_encoder(self):
        """Initialize Depth Anything V2 vision encoder."""
        if not self.use_vision or self.rgb_camera is None:
            print("[G1VisionEnv] Vision disabled or no RGB camera")
            return
        
        try:
            # Resolve checkpoint path
            checkpoint_path = self.vision_cfg.checkpoint_path
            if not checkpoint_path:
                # Default path
                module_dir = Path(__file__).parents[2] / "modules"
                checkpoint_path = str(
                    module_dir / "Depth-Anything-V2" / "metric_depth" / "checkpoints" 
                    / f"depth_anything_v2_metric_hypersim_{self.vision_cfg.encoder}.pth"
                )
            
            # Create encoder
            self.vision_encoder = DepthAnythingEncoder(
                encoder=self.vision_cfg.encoder,
                freeze_encoder=self.vision_cfg.freeze_encoder,
                use_projection=False,  # Projection done in ActorCriticVision
                feature_type=self.vision_cfg.feature_type,
            ...
            self.vision_manager = VisionFeatureManager(
                ...
                update_interval=self.vision_cfg.update_interval,
                device=self.device,
            )
            
            print(f"[G1VisionEnv] Vision encoder initialized:")
            print(f"  - Encoder: {self.vision_cfg.encoder}")
            print(f"  - Feature dim: {self.vision_feature_dim}")
            print(f"  - Update interval: {self.vision_update_interval} steps")
            
        except Exception as e:
            print(f"[G1VisionEnv] Failed to initialize vision encoder: {e}")
            self.vision_encoder = None
            self.vision_manager = None

    def get_vision_features(self, dones: torch.Tensor = None, force_update: bool = False) -> torch.Tensor:
        """
        Get vision features from RGB camera.
        
        Args:
            dones: [num_envs] reset flags (force update for reset envs)
            force_update: Force update all envs regardless of interval
            
        Returns:
            vision_features: [num_envs, vision_feature_dim] extracted features
        """
        if self.vision_manager is None:
            # Return zeros if vision not available
            return torch.zeros(
                self.num_envs, self.vision_feature_dim, 
                device=self.device, dtype=torch.float32
            )
        
        # Get RGB image from camera
        rgb_raw = self.rgb_camera.data.output["rgb"]
        if rgb_raw.shape[-1] == 4:  # RGBA -> RGB
            rgb_raw = rgb_raw[..., :3]
    ...
    def compute_current_observations(self):
        ...
        # Add feet position and velocity in body frame
    ...
    def _compute_feet_state(self):
        ...
        feet_pos_w = robot.data.body_pos_w[:, feet_body_ids, :]
        feet_vel_w = robot.data.body_lin_vel_w[:, feet_body_ids, :]
        
        root_pos_w = robot.data.root_pos_w
        root_vel_w = robot.data.root_lin_vel_w
        root_quat_w = robot.data.root_quat_w
        
        feet_pos_translated = feet_pos_w - root_pos_w.unsqueeze(1)
        feet_vel_translated = feet_vel_w - root_vel_w.unsqueeze(1)
    ...
    def compute_observations(self):
        ...
        # Height scanner observations (privileged for critic)
        if self.cfg.scene.height_scanner.enable_height_scan:
            height_scan = (
            ...
            # Actor only gets height_scan if critic_only=False
    ...
    def reset(self, env_ids=None):
        ...
        # Get initial vision features for reset environments
        if self.use_vision:
            dones = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
            dones[env_ids] = True
            vision_features = self.get_vision_features(dones=dones, force_update=True)
            self.extras["vision_features"] = vision_features
    ...
    def step(self, actions: torch.Tensor):
        ...
        # Get vision features (VisionFeatureManager handles frequency control)
        if self.use_vision:
            vision_features = self.get_vision_features(dones=self.reset_buf)
            self.extras["vision_features"] = vision_features
