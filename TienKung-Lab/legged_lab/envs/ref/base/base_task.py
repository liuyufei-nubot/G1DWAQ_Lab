...
class BaseTask():
    ...
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.obs_history_len = cfg.env.obs_history_len
        ...
        if self.headless == False:
            ...
            self.gym.subscribe_viewer_keyboard_event(
                self.viewer, gymapi.KEY_Z, "gait1")
            self.gym.subscribe_viewer_keyboard_event(
            ...
            for evt in self.gym.query_viewer_action_events(self.viewer):
                ...
                if not self.free_cam:
                    ...
                    if evt.action == "right_turn" and evt.value > 0:
                        self.commands[self.lookat_id, 2] -= 0.5
                    if evt.action == "gait1" and evt.value > 0:
                        self.gait_commands[self.lookat_id, :] = 0
                        self.gait_commands[self.lookat_id, 0] = 1
                    if evt.action == "gait2" and evt.value > 0:
                        self.gait_commands[self.lookat_id, :] = 0
                        self.gait_commands[self.lookat_id, 1] = 1
                    if evt.action == "gait3" and evt.value > 0:
                        self.gait_commands[self.lookat_id, :] = 0
                        self.gait_commands[self.lookat_id, 2] = 1
